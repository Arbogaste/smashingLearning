#!/usr/bin/env python3
"""
ChatterboxTTS (non-turbo) long-form generation via text chunking.
Non-turbo generates ~30-40s per chunk vs Turbo's ~10s.
Requires reference audio for best voice quality.

Run:
  python3 test_nonturbo_long.py --ref path/to/voice.wav
  python3 test_nonturbo_long.py  # uses built-in default voice
"""
...
OUT_DIR = Path("outputs/nonturbo_long")
...
def main() -> int:
    ...
    parser.add_argument("--ref", default=None, help="Reference audio for voice cloning (strongly recommended)")
    parser.add_argument("--max-words", type=int, default=50, help="Max words per chunk")
    ...
    print(f"Text → {len(chunks)} chunks (max {args.max_words} words each)")
    for i, c in enumerate(chunks):
        print(f"  [{i+1}] {len(c.split())}w: {c[:70]}...")

    print(f"\nLoading ChatterboxTTS (non-turbo) on CPU...")
    t_load = time.time()
    try:
    ...
    if args.ref and Path(args.ref).exists():
        ...
        if not args.ref:
            print("No --ref specified, using built-in default voice")
        else:
            print(f"✗ ref file not found: {args.ref}, using built-in voice")
    print()
    ...
    for i, chunk in enumerate(chunks):
        ...
        t0 = time.time()
        try:
            wav = model.generate(chunk, exaggeration=args.exaggeration)
        except Exception as e:
            print(f"  ✗ FAIL: {e}")
        ...
        print(f"  ✓ {dur:.1f}s  elapsed={elapsed:.0f}s  RTF={rtf:.2f}x")
    ...
    ref_tag = Path(args.ref).stem if args.ref else "builtin"
    out_path = OUT_DIR / f"{ts}_{ref_tag}_{len(wav_chunks)}chunks.wav"
    torchaudio.save(str(out_path), full_wav, model.sr)

    print(f"\n{'='*50}")
    print(f"✅ {len(wav_chunks)}/{len(chunks)} chunks")
